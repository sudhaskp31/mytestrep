...
{
    public class UnderstandingDelegate
    {
        public delegate void MyDelegate(int num1, int num2);
        //Creating ref to delegate type
        MyDelegate calDelAdd, calDelProd,calDelDiv,calDelSub;
        public UnderstandingDelegate()
        {
            //takes method as parmeter 
            //only thoe methods which comply with the type def
            calDelAdd = new MyDelegate(Add);
            calDelProd = new MyDelegate(Prod);
            //anonymous method re fby delegate
            ...
            //Action is a predefined delegate type
